...
public class TimestampDeserializer extends JsonDeserializer<Timestamp> {
     ...
     * handles. Returned instance is to be constructed by method itself. <p> Pre-condition for this method is that the
     * parser points to the first event that is part of value to deserializer (and which is never JSON 'null' literal,
     * more on this below): for simple types it may be the only value; and for structured types the Object start marker
     * or a FIELD_NAME. </p>
     * <p>
     * The two possible input conditions for structured types result from polymorphism via fields. In the ordinary case,
     * Jackson calls this method when it has encountered an OBJECT_START, and the method implementation must advance to
     * the next token to see the first field name. If the application configures polymorphism via a field, then the
     ...
     *      }
     *  </pre>
     * Jackson consumes the two tokens (the <tt>@class</tt> field name and its value) in order to learn the class and
    ...
    public Timestamp deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        try {
            return Timestamp.valueOf(DateTimeUtil.parseTimestamp(p.getText()));
        } catch (IOException e) {
